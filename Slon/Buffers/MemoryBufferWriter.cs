...
sealed class MemoryBufferWriter : Stream, IOutputWriter<byte>, ICopyableBuffer<byte>
{
    ...
    public static void Return(MemoryBufferWriter bufferWriter)
    {
        _cachedInstance = bufferWriter;
#if DEBUG
        bufferWriter._inUse = false;
#endif
        bufferWriter.Reset();
    }
    ...
    public void CopyTo<TWriter>(ref TWriter destination) where TWriter : struct, IBufferWriter<byte>, allows ref struct
    {
        if (_completedSegments != null)
        {
            ...
            {
                destination.Write<TWriter, byte>(_completedSegments[i].Span);
            }
        }

        destination.Write<TWriter, byte>(_currentSegment.AsSpan(0, _position));
    }
