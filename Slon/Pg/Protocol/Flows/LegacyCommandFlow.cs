...
internal abstract class LegacyCommandFlowObserver : PgClientFlowObserver
{
    protected internal virtual void OnStarted(LegacyCommandFlow flow, object? state) { }
    protected internal virtual void OnCommandResult(LegacyCommandFlow flow, CommandResult result, object? state) { }
    protected internal virtual void OnDrainStarted(LegacyCommandFlow flow, object? state) { }
}

[Experimental(ExperimentalDiagnostics.PostgreSqlLowerLayer)]
internal readonly struct LegacyCommandFlowOptions
{
    public LegacyCommandFlowObserver? Observer { get; init; }
...
[Experimental(ExperimentalDiagnostics.PostgreSqlLowerLayer)]
internal partial class LegacyCommandFlow : PgClientFlow, IValueTaskSource<bool>, IValueTaskSource<FlowCallerInteractionCoreResult>, IValueTaskSource
{
    ...
    // optimization that needs these fields is LegacyCommandFlow-specific (see DispatchPipelinedRead).
    ...
    LegacyCommandFlow() : base(supportsDeferredFlush: true)
    {
    ...
    internal LegacyCommandFlow(bool async, params ReadOnlySpan<Command> commands) : this()
        => Initialize(async, commands);
    internal LegacyCommandFlow(bool async, in LegacyCommandFlowOptions options) : this()
        => Initialize(async, options);

    private protected LegacyCommandFlow(bool async, TimeSpan? pendingTimeout) : this()
    {
        IsAsync = async;
        _pendingTimeout = pendingTimeout;
    }

    internal LegacyCommandFlow Initialize(bool async, params ReadOnlySpan<Command> commands)
        => Initialize(async, options: new() { Commands = new(commands) });

    internal LegacyCommandFlow Initialize(bool async, in LegacyCommandFlowOptions options)
    {
        IsAsync = async;
    ...
    FlowTasks ExecuteAutoCore(Context context)
    {
        ...
        return new FlowTasks(
            ...
            pipelineTask: DispatchPipelinedRead(
                context, context.GetProtocolStatic<CommandFlow.ReadPromiseState>().Promise));
    }
    ...
    ValueTask DispatchPipelinedRead(Context context, ValueTaskSourcePromise<bool> promise)
    {
        ...
        {
            var flow = (LegacyCommandFlow)state!;
            ...
            var promise = ctx.GetProtocolStatic<CommandFlow.ReadPromiseState>().Promise;
            PromiseAsyncValueTaskMethodBuilder.Promise = promise;
                    ...
                    ((IValueTaskSource)promise).OnCompleted(static state =>
                    {
                        ...
                        try
                        {
                            var promise = flow._context
                                .GetProtocolStatic<CommandFlow.ReadPromiseState>().Promise;
    ...
    async ValueTask ExecutePipelined(Context context)
    {
        ...
        try
        {
            ...
            while (++_commandIndex < CommandCount)
            {
                ...
                {
                    var rowDescription = context.GetProtocolStatic<CommandFlow.ReadState>().RowDescription;
                ...
                ((LegacyCommandFlowObserver?)GetObserver(out var observerState))
                ...
                {
                    var resultEnumerator = context.GetProtocolStatic<CommandFlow.ReadState>().ResultMessageEnumerator;
    ...
    ValueTask ReadPreparationDescription(Context context, PreparationReadState state)
    {
        ...
        static async ValueTask AwaitRead(
            LegacyCommandFlow flow, PreparationReadState state,
    ...
    void RegisterCancellationCallbacksLocked(CancellationState cancellation)
    {
        if (cancellation.CallerToken.CanBeCanceled)
        {
            ...
            if (cancellation.CallerRegistration == default)
                cancellation.CallerRegistration = cancellation.CallerToken.UnsafeRegister(static (state, token)
                    => ((LegacyCommandFlow)state!).RequestCancelAndWake(token, CancellationScope.CurrentWindow), this);
        }
        if (cancellation.FlowToken.CanBeCanceled && cancellation.FlowRegistration == default)
        {
            cancellation.FlowRegistration = cancellation.FlowToken.UnsafeRegister(static (state, token)
                => ((LegacyCommandFlow)state!).RequestCancelAndWake(token, CancellationScope.RemainingFlow), this);
        }
    }
    ...
    static ref FlowCallerInteractionCore<FlowCallerInteractionCoreResult> GetCallerInteractionCore(LegacyCommandFlow instance)
