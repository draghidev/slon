...
partial class PgTypeCatalog
{
    ...
    static IEnumerable<PgType> CreateDefaultTypes(bool withMultiranges)
    {
        ...
        {
            var type = PgType.CreateBase(name, oid);
            types.Add(type);
            if (arrayOid is not 0)
                types.Add(PgType.CreateArray(type, arrayOid));
        }
        ...
        {
            var type = PgType.CreateBase(name, oid);
            var rangeType = PgType.CreateRange(type, rangeName, rangeOid);
            types.Add(type);
            types.Add(PgType.CreateArray(type, arrayOid));
            types.Add(rangeType);
            types.Add(PgType.CreateArray(rangeType, rangeArrayOid));
            if (withMultiranges)
            {
                var multirangeType = PgType.CreateMultirange(rangeType, rangeName.ToDefaultMultirangeName(), multirangeOid);
                types.Add(multirangeType);
                types.Add(PgType.CreateArray(multirangeType, multirangeArrayOid));
            }
        }
    }
}
