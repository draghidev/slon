...
public class SyncTeardownTests
{
    ...
    public async Task SyncRead_ParkedAwaitingResponse_ForcefulDispose_SeesClosed()
    {
        ...
        // Let the query reach the wire and the read park on the (withheld) response. ~3x the real
        // localhost park time (~10ms); the test doesn't depend on the exact value, only that the read is
